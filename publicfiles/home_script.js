...
async function loadschedule(datas, role, rec, curr_id) {
  ...
  datas.forEach(async (data, index) => {
    ...
    const diff = await dateDifference(data.date, new Date(), data.time_in)
...
function dateDifference(dateStr, date2, time) {
  ...
  // Set the hours and minutes to the provided date
  let finalHR
  switch (hours) {
    case 1:
      finalHR = 13;
...
function dateModify(date, time) {
  // Split the provided time string into hours and minutes
  ...
  // Calculate the difference in hours between the modified date and current date
  const diffInMs = date - currentDate;
  const diffInHours = diffInMs / (1000 * 60 * 60);

  // Check if the modified date is still 2 hours ahead of the current time
  if (diffInHours >= 2) {
    return true;
  } else {
    return false;
