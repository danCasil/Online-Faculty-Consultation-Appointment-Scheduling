...
const pm_options = ['1:00', '1:30', '2:00', '2:30', '3:00', '3:30', '4:00', '4:30', '5:00']
...
function checkconflict() {
  ...
  if (document.getElementById("time1am").value == 'pm') {
    ...
    timeout = `${am_options[btn1.selectedIndex + 1]}:00`
    ...
    .then(data => {
      ...
      if (data) {
        ...
        if (data.datas && data.datas.length > 0) {
          const tin1 = convertToTime(timein); const tout1 = convertToTime(timeout); data.datas.forEach(frag => {
            const tin2 = convertToTime(frag.timein); const tout2 = convertToTime(frag.timeout); // Check if there is any overlap
            if ((tin2 < tout1 && tout2 > tin1) || (tin2 < tin1 && tout2 > tout1) || (tin2 >= tin1 && tout2 <= tout1)) { isConflict = true; }
          });
        } // Update UI based on conflict status
        const saveChangesButton = document.getElementById("saveChanges");
        if (isConflict) {
          saveChangesButton.textContent = "Conflicted Time"; saveChangesButton.disabled = true;
        }
        else {
          saveChangesButton.textContent = "Save changes"; saveChangesButton.disabled = false;
        }
      }
    })
    .catch(err => {
...
function removethis(index) {
  ...
  if (confirmed) {
    fetch(`/RemoveThis?id=${index}`).then(response => response.json()).then(data => {
...
function formatTime(time) {
  const [hours, minutes, seconds] = time.split(':'); let period; 
  if (hours >= 1 && hours < 6) {
       period = 'PM'; } 
  else if (hours >= 8 && hours < 12) {
     period = 'AM'; } 
